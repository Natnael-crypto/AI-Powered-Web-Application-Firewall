...
function DashboardMenu({changeMenu, selectedMenu}: dashboardMenuProps) {
  return (
    <div className=" flex items-center py-2 px-2 gap-3 shadow-md bg-white rounded-md">
      <Button
        classname={`px-4 py-1 ${selectedMenu == 'basic' ? 'bg-green-500' : 'border border-1 border-gray-400'} rounded-lg text-lg `}
        onClick={changeMenu}
      >
        Basic
      </Button>
